...
1. API routes, controller 분리
2. API 설계단계부터
3. NOSQL => SQL (MariaDB)
4. Session => JWT 토큰방식
5. cryto를 사용한 Incoding
...
- 회원가입받기
  Method: POST
  URL: "/join"
  http status code: 201(통과), 400(실패)
  req: userId, userPassword, nickName
  res:
  > 2. 로그인
- 로그인요청
  Method: POST
  URL:"/login"
  http status code: 200(통과), 400(실패)
...
- 게시글 조회
  ...
  http status code: 200(통과) ,400(실패)
  req:
  res: title, contents, publishedDAte, category, userId
- 게시글 작성
  Method: POST
  URL:"/board"
  http status code: 201(통과), 400(실패)
  req: userId, boardId, title, contents, publishedDate, category
  res:
- 게시글 삭제
  Method: DELETE
  URL:"/board/:boardId"
...
- 댓글 작성
  ...
  URL:"/chat"
  http status code: 201(통과), 400(실패)
  req: userId, boardId, chatId, comment
  res:
- 댓글 삭제
  Method: DELETE
  URL:"/chat:/chatId"
